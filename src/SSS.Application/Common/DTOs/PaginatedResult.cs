...
namespace SSS.Application.Common.Dtos
{
    public class PaginatedResult<T>
    {
        ...
        public PaginatedResult(int pageIndex, int pageSize, int totalCount, List<T> items)
        {
            ...
            IQueryable<T> source, int pageIndex, int pageSize, CancellationToken ct = default)
        {
            ...
            return new PaginatedResult<T>(pageIndex, pageSize, count, items);
        }

        // đổi Items sang kiểu khác nhưng giữ metadata
        public PaginatedResult<TOut> MapItems<TOut>(Func<T, TOut> map)
